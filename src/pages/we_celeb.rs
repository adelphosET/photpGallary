...
#[component]
pub fn HomeWeddingPic(idid: String) -> Element {
    let mut selected = use_signal(|| None::<usize>);

    let images_res = use_server_future({
        let idid = idid.clone();
        move || {
            let value = idid.clone();
            async move { get_by_category(value).await }
        }
    })?;
...
// #[component]
// pub fn HomeWeddingPic(category: String) -> Element {
//     let mut selected = use_signal(|| None::<usize>);

//     let images_res = use_server_future(move || {
//         let category = category.clone();
//         async move { get_by_category(category).await }
//     })?;
//     rsx! {
//         div { class: "max-w-6xl mx-auto p-4 scroll-smooth",
//             match  (images_res.value())() {
...
//                                 idx,
//                                 on_close: move |_| *selected.write() = None,
//                                 on_next: move |_| *selected.write() = Some((idx + 1) % img_assets.len()),
//                                 on_prev: move |_| *selected.write() = Some((idx + img_assets.len() - 1) % img_assets.len()),
//                             }
//                         }
//                     }
//                 }
//                 Some(Ok(_)) => rsx!(p { "No images found for this category." }),
